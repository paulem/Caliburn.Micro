...
namespace Caliburn.Micro.WPF.Tests
{
    ...
    public class RegExHelperTests
    {
        ...
        public void ShouldInsertRegexWhenNamespaceHasAsterix()
        {
            ...
            Assert.Equal(
                @"Caliburn\.([\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}_][\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}\p{Mn}\p{Mc}\p{Nd}\p{Pc}\p{Cf}_]*\.)*WPF\.Tests",
                result);
        }
        ...
        public void GetNameCaptureGroupShouldUseNameRegEx()
        {
            ...
            Assert.Equal(
                @"(?<group>[\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}_][\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}\p{Mn}\p{Mc}\p{Nd}\p{Pc}\p{Cf}_]*)",
                result);
        }
